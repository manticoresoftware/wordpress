...
class TopSearchesWidget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'TopSearchesWidget', 'Manticore Related/Top Searches', [
			'classname'   => 'TopSearchesWidget',
			'description' => 'Manticore related/top search terms'
		] );
	}
	...
	function widget( $args, $instance ) {
		...
		$period_limit     = ! empty( $instance['period_limit'] ) ? (int) $instance['period_limit'] : '';
		...
		if ( is_search() && $search_show != 'hide' ) {
			...
			$show_widget = true;
		} elseif ( is_singular() && $posts_show != 'hide' ) {
	...
	function get_top(
		...
		$result = $defaultObjectSphinxSearch::$plugin->frontend->sphinx_stats_top( $limit, $width, $break,
			$show_approved,
			$period_limit );
		if ( empty( $result ) ) {
			$html .= "</ul>";
	...
	function get_related( $keywords, $limit = 10, $width = 0, $break = '...', $show_approved = false ) {
		...
		$result = $defaultObjectSphinxSearch::$plugin->frontend->sphinx_stats_related( $keywords, $limit, $width,
			$break,
			$show_approved );
		if ( empty( $result ) ) {
			return false;
